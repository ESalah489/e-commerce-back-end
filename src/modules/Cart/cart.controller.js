...
const addToCart = async (req, res, next) => {
    try{
        ...
        // res.status(200).json({ message: 'Product added to cart successfully', product: addedProduct });
...
const updateQuantity = async (req, res, next) => {
    try {
        ...
        if (!productId || quantity < 1){
            return res.status(404).json({message : "Valid product ID and quantity required"})
        } 
...
export default {addToCart, getCart, updateQuantity, removeFromCart, clearCart}